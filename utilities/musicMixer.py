...
def createMix(song1, tempo1, song2, tempo2, MInfo, output, easeCalc = 0):
    ...
    soundfile.write("temp/track1.wav" if output else "temp/track2.wav", yTotal.T, sr)
    print("Mix", "temp/track1.wav" if output else "temp/track2.wav", "created")
...
for file in os.listdir(path):
    if (
        ...
        songs.append("tracks/" + file)

print(songs)

tempoLookUp = {
    'Pohjola.wav' : 104, 
    'Sellainen Tää Maailma On - Bablo.wav' : 122, 
    'Octane.wav' : 124, 
...
class MixInfo:
    ...
    def startPlaying(self, firstTrack = True, nextup = ""):
        ...
        if firstTrack:
            self.lastSong = "tracks/gamebegin.wav"
        else:
            self.lastSong = random.choice(songs)
        self.tempo = 130
        if nextup:
        ...
        pygame.mixer.music.load("temp/track1.wav" if self.output else "temp/track2.wav")
...
def threadedMixCreation(MInfo):
    ...
    pygame.mixer.music.queue("temp/track1.wav" if MInfo.output else "temp/track2.wav")
